...
# ----------代码大概就是三部分：数据读取 → 核心训练 → 封装配置------------------
# 数据读取：read_texts_from_jsonl 

'''加载训练数据,读取JSONL⽂件并安全提取⽂本数据'''
# Generator[产出类型, 发送类型, 返回类型]
# str代表每次生成的数据类型，第一个None代表外部不传值进来，
# 第二个None代表函数全部执行完毕，没有下一行了时，没有 return，所以是None
...
def create_tokenizer_config(save_dir: str) -> None:
    config = {
        ...
        # chat_template 规定了如何将一个 Python 列表（包含 role 和 content）转换成模型能看懂的“一长串字符串”
        "chat_template": (
...
def train_tokenizer(data_path: str, save_dir: str, vocab_size: int = 8129) -> None:
    ...
    # train_from_iterator是HuggingFace tokenizers库（底层用 Rust 编写）暴露给Python接口
    # 在底层（C++ 或 Rust 级别）内部运行着一个 while 循环，一次次地取texts
    '''
...
def main():
    ...
    save_dir = "E:\\github\\Happy_llm_learning\\Chapter5\\Pretrain_code\\tokenizer_k"
    # 训练tokenizer
    # train_tokenizer(
    #     data_path=data_path,
    #     save_dir=save_dir,
